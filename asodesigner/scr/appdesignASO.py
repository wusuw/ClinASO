...
def get_ortholog_gene_ids(ortholog_file, human_gene_id, target_taxids, species_map):
    ...
    # 构建taxid到common_name的映射，提高查找速度
    taxid_to_name = {taxid: name for name, taxid in species_map.items()}
    
    # 使用mmap提高大文件读取速度
    with open(ortholog_file, 'r+b') as f:
        # 将文件映射到内存
            ...
            try:
                ...
                if cols[1] == human_gene_id:
                    ...
                    if target_taxid in target_taxids:
                        # 直接通过taxid查找common_name
                        if target_taxid in taxid_to_name:
                            common_name = taxid_to_name[target_taxid]
    ...
    logger.info(f"Ortholog lookup completed in {elapsed:.2f} seconds, found {len(orthologs)} orthologs")
    return orthologs
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'rb') as f:
                    cache = pickle.load(f)
                logger.info(f"Loaded GTF cache from {cache_file}")
            except Exception as e:
            ...
            try:
                ...
                try:
                    with open(cache_file, 'wb') as f:
                        pickle.dump(cache, f)
                    logger.info(f"Saved GTF cache to {cache_file}")
                except Exception as e:
...
def main(human_gene_id, species_file, ortholog_file, output_dir):
    ...
    # 设置最大工作进程数（对于双核4G服务器，使用1个进程以减少内存使用）
    max_workers = 1
    logger.info(f"Using {max_workers} worker process")
    ...
    elapsed = time.time() - start_time
    logger.info(f"Processing completed! Success: {success_count}/{len(tasks)} species, Total time: {elapsed:.2f} seconds")
