...
def process_species(args):
    ...
    try:
        ...
        # 获取基因位置（不使用缓存，减少内存使用）
        location = get_gene_location(gtf_file, gene_id)
        if not location:
        ...
        sequence = extract_sequence(genome_file, chrom, start, end, strand)
        if not sequence:
            logger.error(f"Chromosome {chrom} not found in {species} genome or empty sequence extracted")
            return False
            
        # 限制序列长度，减少内存使用
        max_seq_length = 1000000  # 限制最大序列长度为1MB
        if len(sequence) > max_seq_length:
            logger.warning(f"Sequence for {species} is too long ({len(sequence)} bases), truncating to {max_seq_length} bases")
            sequence = sequence[:max_seq_length]
        
        # 创建SeqRecord对象
        record_id = f"{species}_{gene_id}"[:200]  # 限制长度
        description = f"_{species}_ortholog_of_{human_gene_id}"[:200].replace('\n', '')+"_"+"_".join([chrom, str(start), str(end), strand])
...
# 从GTF文件中获取基因位置信息（不使用缓存）
def get_gene_location(gtf_file, target_gene_id):
    try:
        with open(gtf_file, 'r') as f:
            for line in f:
                if line.startswith('#'):
...
def extract_sequence(genome_file, chrom, start, end, strand):
    try:
        ...
        logger.error(f"Error extracting sequence from {genome_file}: {str(e)}")
        return None
